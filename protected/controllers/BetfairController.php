...
class BetfairController extends Controller
{
  ...
  public function actionMarkets($eventId)
  {
    ...
    $this->render('markets',array('markets'=>$markets, 'marketId'=>$eventId));
  }
  ...
  public function actionMarketViewer($marketId)
  {
    ...
    //$this->render('markets',array('markets'=>$markets, 'dataProvider'=>$dataProvider));
    $this->render('marketViewer',array('marketDetails'=>$marketDetails, 'marketOdds'=>$marketOdds, 'marketId'=>$marketId));
  }

  public function actionOddViewer($marketId)
  {
    // session cookie is set by betfair->login
    $session = Yii::app()->request->cookies['session']->value;
    $model = new Betfair;
    ...
    //$this->render('markets',array('markets'=>$markets, 'dataProvider'=>$dataProvider));
    //$this->render('oddViewer',array('marketDetails'=>$marketDetails, 'marketOdds'=>$marketOdds));
    $this->renderPartial('_oddViewer',array('marketOdds'=>$marketOdds));
    //$this->render('oddViewer');
  }

  public function actionGetOdds($marketId)
  {
    $session = Yii::app()->request->cookies['session']->value;
    $model = new Betfair;
    $marketOdds = $model->marketOdds($session, $marketId);

    $this->renderPartial('oddViewer',array('marketOdds'=>$marketOdds));  
     //$this->render('oddViewer',array('marketOdds'=>$marketOdds));  
  }
